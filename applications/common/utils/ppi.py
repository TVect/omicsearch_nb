...
from omicverse.utils import plot_network
...
def calc_hub_gene(
    ...
    enr_fig = None
    # params = {
    #     "identifiers": "\n".join(set(gene_mapping["stringId"].values)), 
    #     "species": 9606, 
    #     "show_query_node_labels": 1
    # }
    # string_url = f'https://string-db.org/cgi/network?{urlencode(params)}'

    gene_type_dict = {item["gene"]: "Type1" for item in records}
    gene_color_dict = {item["gene"]: "#F7828A" for item in records}
    ppi_graph = plot_network(ppi_graph, gene_type_dict, gene_color_dict)


    pathway_dict=ov.utils.geneset_prepare(
        '/mnt/sano6-1/qxl/Workspaces/11-web/FlaskProjects/omicsearch_nb/genesets/WikiPathway_2021_Human.txt',
        organism='Human'
    )
    enr = ov.bulk.geneset_enrichment(
        gene_list=[item["gene"] for item in records],
        pathways_dict=pathway_dict,
        pvalue_type='auto',
        organism='human'
    )
    enr_fig = ov.bulk.geneset_plot(enr, figsize=(3, 5)).figure

    return records, ppi_graph, enr_fig
